...
Date = '161025'
Name = 'Mercury_Yellow_Filter_1.2'
file = rf'C:\Users\zhaow\OneDrive - Imperial College London\Labs\Y2\Interferometry\Data\{Date}_{Name}.txt'
savefigbool = 1

file = r"C:\Users\zhaow\OneDrive - Imperial College London\Documents\interferometry_data_20251030_094056.txt"

results = rd.read_data3(file)

# Describe the global calibration used (from either Task 6, or crossing_points.py)
metres_per_microstep = 3.75346e-11/2 # metres
...
plt.figure()
plt.plot(x,y1,'-')
# plt.xlim(1.5e-5,2.1e-5)
plt.xlabel("Position $\mu$steps")
plt.ylabel("Signal 1")
# plt.savefig(rf'C:\Users\zhaow\OneDrive - Imperial College London\Labs\Y2\Interferometry\Figures\{Date}_{Type}_{Version}_interferogram.png')
...
N = 2**15 # these are the number of points that you will resample - try changing this and look how well the resampling follows the data.
xs = np.linspace(x[0], x[-1], N) # x-axis to resample onto
y = y1[:len(x)] # make sure y axis has same length as x 
cs = spi.UnivariateSpline(x, y, s=0) # construct the cubic spline function
...
# plt.savefig(rf'C:\Users\zhaow\OneDrive - Imperial College London\Labs\Y2\Interferometry\Figures\{Date}_{Type}_{Version}_spectrum_from_global_calibration.png')
plt.show()
